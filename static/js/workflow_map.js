// --- Page Variables --- //
// initialize the map
var map = L.map("map", { renderer: L.svg({ padding: 100 }) }).setView(
    [40.265306, -98.623725],
    5
);
...
function onEachFeatureClick(feature, layer) {
    console.log("onEachFeatureClick:", feature, layer);
    layer.on("click", function (e) {
        if (
            ...
            $("#hucID").html(
                "Huc 8 ID: <a href='https://cfpub.epa.gov/surf/huc.cfm?huc_code=" +
...
function selectHUCs(hucs, lat, lng) {
    ...
    if (
        layerSelected.localeCompare("catchment") === 0 &&
        secondaryInputLayer === ""
    ) {
        secondaryInputLayer = L.tileLayer
            .wms(
        ...
        var url = "https://api.epa.gov/waters/v1/pointindexing";
        var ptIndexParams = {
...
function addStreamSeg(streamData) {
    var latlon = streamData.output.ary_flowlines[0].shape.coordinates.map(
        function (c) {
            return c.reverse();
        }
    );
...
function binarySearch(left, right, value) {
    while (left <= right) {
        ...
        if (currentInputLayer.hasLayer(mid)) {
            var midValue = Number(
                currentInputLayer.getLayer(mid).feature.properties.HUC_8
            );
...
function updateInputLayer() {
    ...
    if (layerSelected.localeCompare("none") === 0) {
        ...
        $("#input-description").html(
            "Select a huc 8 by clicking on the map or entering a huc 8 ID in the search box."
        );
        ...
        $("#input-description").html(
            "Select a huc 8 by clicking on the map or entering a huc 8 ID in the search box. Once a huc has been selected zoom into your area of interest and select the stream segment for your catchment."
        );
        $("#inputSearchBlock").show();
        $("#geometry-title").trigger("click");
...
function getData() {
    ...
    var source = $("#source-input").val();
    var temporalResolution = document.getElementById("temporal-resolution")
        .options[document.getElementById("temporal-resolution").selectedIndex]
        .value;
    var timeLocalized =
        document.getElementById("time-localized").options[
            document.getElementById("time-localized").selectedIndex
        ].value;
    var requestData;
    var selection = document.getElementById("inputLayer");
    var layerSelected = selection.options[selection.selectedIndex].value;
    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            workflowData = data;
            $("#data-request-success").html(
                "Successfully downloaded workflow data."
            );
            setMetaTable();
            populateDataTable();
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            $("#data-request-error").html(
                "Error downloading workflow data. " + errorThrown
            );
        },
...
function downloadDataAsCSV() {
    ...
    element.setAttribute(
        "href",
        "data:application/csv;charset=utf-8," + encodeURIComponent(csvData)
    );
